...
namespace FaineSwitch
{
    ...
    {
        public static string logdir = Path.Combine(SwitcherUI.nPath, "Logs");
        ...
        {
            if (!SwitcherUI.LoggingEnabled) return;
            if (!File.Exists(Configs.filePath))
                Program.MyConfs = new Configs();
            if (!Directory.Exists(Path.Combine(SwitcherUI.nPath, "Logs")))
