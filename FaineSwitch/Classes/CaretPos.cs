...
namespace FaineSwitch
{
    public static class CaretPos
    {
        ...
        public static Point GetCaretPointToScreen(out Point caretOnlyPos)
        {
            ...
            string _clsNMfw = "";
            Logging.Log("_c HWND: [" + Program.SWITCHER_HANDLE + "], _c ThrId: [" + _cThr_id +
            ...
            if (_fwThr_id != _cThr_id)
            {
                ...
                if (_clsNMfw == "PX_WINDOW_CLASS" && SwitcherUI.MCDSSupport)
                {
                    System.Threading.Tasks.Task.Factory.StartNew(GetDataFromMCDS);
                    var CaretToScreen = new Point(_fwFCS_Re.Left, _fwFCS_Re.Top);
                    CaretToScreen.X += _CaretST3.X + SidebarWidth + SwitcherUI.MCDS_Xpos_temp;
                    if (viewID == 4)
                    {
                        WinAPI.RECT clts = new WinAPI.RECT(0, 0, 0, 0);
                        WinAPI.GetWindowRect(WinAPI.GetForegroundWindow(), out clts);
                        CaretToScreen.Y = clts.Bottom - SwitcherUI.MCDS_BottomIndent_temp - 45 + SwitcherUI.MCDS_Ypos_temp;
                        CaretToScreen.X -= 20;
                    }
                    else
                        CaretToScreen.Y += _CaretST3.Y + SwitcherUI.MCDS_TopIndent_temp + SwitcherUI.MCDS_Ypos_temp;
