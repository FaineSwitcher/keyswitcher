...
namespace FaineSwitch
{
    static class KInputs
    {
        ...
        {
            var WORDS = FaineSwitch.KMHook.SplitWords(LINE);
            string word = "";
            var len = 0;
        ...
        public static WinAPI.INPUT[] AddString(string str)
        {
            ...
            foreach (var s in str)
            {
                ...
                ushort resultvk = 0;
                short lt1_vk = WinAPI.VkKeyScanEx(s, FaineSwitch.SwitcherUI.MAIN_LAYOUT1);
                uselt1_vk = lt1_vk != -1;
                short lt2_vk = WinAPI.VkKeyScanEx(s, FaineSwitch.SwitcherUI.MAIN_LAYOUT2);
                uselt2_vk = lt2_vk != -1;
                if (uselt1_vk && uselt2_vk && lt1_vk != lt2_vk)
                {
                    var guess = FaineSwitch.KMHook.WordGuessLayout(GetWordByIndex(str, index));
        ...
        { //Simply, sends input
            ...
            if (done != sinputs.Length)
                FaineSwitch.Logging.Log("ERROR during send input, lenght: " + done + ", Win32ERR: " + Marshal.GetLastWin32Error());
        }
    }
}