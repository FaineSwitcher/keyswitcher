...
public class Languages
{
    ...
    public static Dictionary<Element, string> English = new Dictionary<Element, string>() { 
        ...
        { Element.JustIcon, "FaineSwitch's icon" },
        { Element.ContryFlags, "Country flags" },
        { Element.TextLayout, "Text of layout" },
        { Element.BlockCtrlHKs, "Block FaineSwitch hotkeys with Ctrl." },
        ...
        { Element.TT_ConfigsInAppData, "If enabled FaineSwitch will copy current configs to AppData, and will use them.\r\nAlso logs and snippets will be stored in %AppData%\\FaineSwitch.\r\nAfter this checkbox state changed other checkboxes/datas configurations from user interface will not be saved, because they will be loaded from another configs(if switched, from FaineSwitch's directory or from %AppData%\\FaineSwitch).\r\nUseful if you need to run FaineSwitch from Program Files directory by multiple users, and while some of them have no write access to it,\r\nalso it makes possible to have different configurations for each user." },
        ...
        { Element.TT_UseJKL, "Allows FaineSwitch to retrieve layout from jkl.exe.\r\nAllows always to get right keyboard layout.(Highly recommended)\r\n" },
...
Specify layout code 0 to disable this feature." },
    ...
    public static Dictionary<Element, string> Ukrainian = new Dictionary<Element, string>() {
        ...
        { Element.JustIcon, "Значок FaineSwitch" },
        ...
        { Element.ToggleSwitcher, "Перемкнути паузу FaineSwitch" },
        ...
        { Element.UpdateSwitcher, "<- Оновити FaineSwitch" },
        ...
        { Element.About, "Горячі клавіші: Ви можете подивитися їх в вкладці гарячі клавіші.\r\n"+
            ...
            "\r\nПочитайте wiki або запитайте мене, якщо у вас є питання щодо FaineSwitch (ел. пошта і посилання на wiki нижче).\r\nУдачі."},
		#endregion
		#region Sync
        ...
        { Element.UpdateComplete, "FaineSwitch успішно оновлено!" },
        { Element.ShowHide, "Показати" },
        { Element.Switcher, "FaineSwitch (магія) – відбудований перемикач розкладок." },
        ...
        { Element.TT_Logging, "Призначено ТІЛЬКИ для пошуку помилок, впливає БІЛЬШЕ НІЖ НА ШВИДКІСТЬ РОБОТИ, журнали зберігаються у папці [Logs] поруч з FaineSwitch.exe." },
        ...
        { Element.TT_OneLayout, "Дозволяє зберігати розкладку в FaineSwitch замість розкладки для кожного вікна/програми.\r\n(якщо у вас Windows 8 і вище, то там дана функція вже стоїть за замовчуванням, немає необхідності включати її ще і в FaineSwitch)"},
        ...
        { Element.TT_ConfigsInAppData, "Якщо включено, FaineSwitch скопіює поточну конфігурацію (FaineSwitch.ini) в AppData, і буде використовувати звідти.\r\nТакож логи і сніппети будуть зберігатися в %AppData%\\FaineSwitch.\r\nКоли стан цієї галочки змінено, всі інші галочки/дані конфігурації, змінені в користувальницькому інтерфейсі, не будуть збережені,\r\nт.к. відбудеться зміна конфігурації і завантаження обраної (із папки FaineSwitch або з %AppData%\\FaineSwitch).\r\nКорисно, якщо потрібно запускати FaineSwitch в папці Program Files, а прав на запис у всіх користувачів - немає,\r\nі ще це дає можливість мати різні налаштування для кожного користувача." },
        { Element.TT_KeysType, "Виберіть, який тип клавіш відображати в FaineSwitch, ОБИДВА типи працюють ОДНОЧАСНО,\r\nтак що краще не призначати однакові клавіші/гор. клавіші для уникнення подвійного перемикання розкладки."},
        ...
        { Element.TT_WriteInputHistory, "Буде писати історію введення в history.txt (в папці FaineSwitch або в AppData – в залежності від налаштувань),\nз підтримкою клавіші <Backspace>!, хоча може працювати повільно, якщо історія занадто велика." },
        { Element.TT_UseDelayAfterBackspaces, "Якщо включено, FaineSwitch буде чекати деякий час після видалення старого слова і перед введенням конвертованого слова.\r\nКорисно, якщо в деяких програмах функція FaineSwitch «Конвертація останнього слова» не працює нормально." },
...
Можна вказати навіть ту розкладку, яка не завантажена в системі, наприклад English (US) = 67699721; FaineSwitch завантажить/вивантажить її при натисканні/відпусканні LCtrl+LAlt.
Вкажіть код розкладки 0, щоб вимкнути цю функцію." },
    ...
    public static Dictionary<Element, string> Russian = new Dictionary<Element, string>() {
        ...
        { Element.ToggleSwitcher, "Переключать паузу FaineSwitch" },
        ...
        { Element.UpdateSwitcher, "<- Обновить FaineSwitch" },
        ...
        { Element.About, "Горячие клавиши: Вы можете посмотреть их во вкладке горячие клавиши.\r\n"+
            ...
            "\r\nПочитайте wiki или спросите меня, если у вас есть возникли вопросы по поводу FaineSwitch (эл. почта и ссылка на wiki ниже).\r\nУдачи."}, 
		#endregion
		#region Sync
        ...
        { Element.UpdateComplete, "FaineSwitch успешно обновлен!" },
        { Element.ShowHide, "Показать" },
        { Element.Switcher, "FaineSwitch (魔法) – волшебный переключатель раскладок." },
        ...
        { Element.TT_Logging, "Предназначено ТОЛЬКО для поиска ошибок, оказывает БОЛЬШОЕ ВЛИЯНИЕ НА СКОРОСТЬ РАБОТЫ, журналы сохраняются в папке [Logs] рядом с FaineSwitch.exe." },
        ...
        { Element.TT_OneLayout, "Позволяет хранить раскладку в FaineSwitch вместо раскладки для каждого окна/программы.\r\n(если у вас Windows 8 и выше, то там данная функция уже стоит по умолчанию, нет необходимости включать ее ещё и в FaineSwitch)"},
        ...
        { Element.TT_ConfigsInAppData, "Если включено, FaineSwitch скопирует текущую конфигурацию (FaineSwitch.ini) в AppData, и будет использовать оттуда.\r\nТакже логи и сниппеты будут храниться в %AppData%\\FaineSwitch.\r\nКогда состояние этой галочки изменено, все другие галочки/данные конфигурации, измененные в пользовательском интерфейсе, не будут сохранены,\r\nт.к. произойдет смена конфигурации и загрузка выбранной (из папки FaineSwitch или из %AppData%\\FaineSwitch).\r\nПолезно, если нужно запускать FaineSwitch в папке Program Files, а прав на запись у всех пользователей - нет,\r\nи еще это даёт возможность иметь разные настройки для каждого пользователя." },
        { Element.TT_KeysType, "Выберите, какой тип клавиш отображать в FaineSwitch, ОБА типа работают ОДНОВРЕМЕННО,\r\nтак что лучше не назначайте одинаковые клавиши/гор. клавиши во избежание двойного переключения раскладки."},
        ...
        { Element.TT_WriteInputHistory, "Будет писать историю ввода в history.txt (в папке FaineSwitch или в AppData – в зависимости от настроек),\nс поддержкой клавиши <Backspace>!, хотя может работать медленно, если история слишком большая." },
        { Element.TT_UseDelayAfterBackspaces, "Если включено, FaineSwitch будет ждать некоторое время после удаления старого слова и перед вводом конвертированного слова.\r\nПолезно, если в некоторых программах функция FaineSwitch «Конвертация последнего слова» не работает нормально." },
...
Можно указать даже ту раскладку, которая не загружена в системе, например English (US) = 67699721; FaineSwitch загрузит/выгрузит её при нажатии/отпускании LCtrl+LAlt.
