...
namespace FaineSwitch
{
    ...
    {
        public static readonly string speech_dir = Path.Combine(Path.GetTempPath(), "SwitcherGTSpeech");
        ...
        {
            txt_Source.Text = Program.Lang[Languages.Element.Error] + " " + msg;
            pan_Translations.Width = 0;
            pan_Translations.Height = 0;
            Height = TITLE.Height + 1 + txt_Source.Height + 2;
            SetOptimalWidth();
        ...
        public static List<GTResp> GetGTResponceAll(string[] tls, string[] qs, string[] sls)
        {
            ...
            catch (Exception e) { Program.switcher._TranslatePanel.GTRespError(e.Message/*+e.StackTrace*/); }
            return gtrlist;
        }
        public void ShowTranslation(string str, Point pos)
        {
            ...
            running = true;
            var tls = (from pv in SwitcherUI.TrSetsValues
                       where pv.Key.StartsWith("cbb_to")
                       select pv.Value).ToArray();
            var sls = (from pv in SwitcherUI.TrSetsValues
            ...
            else
            {
                ...
                if (gtrs.Count > 0)
                {
                    TITLE.Text = Program.Lang[Languages.Element.Translation] + " <" + gtrs[0].src_lang + ">";
                    TITLE.Font = SwitcherUI.TrTitle;
        ...
        public void AddTranslation(GTResp gtr)
        {
            ...
            if (TRANSCRIPTION)
            {
                ...
                {
                    var txt = new SwitcherUI.TextBoxCA();
                    txt.Name = txtstrc;
                    txt.Multiline = true;
                    if (this.Controls.ContainsKey(txtstrc))
                    {
                        txt = (SwitcherUI.TextBoxCA)this.Controls[txtstrc];
                    }
                    else
                    {
                        this.Controls.Add(txt);
                    }
                    txt.Text = "[" + SwitcherUI.UnescapeUnicode(gtr.src_transcr) + "]";
                }
                else
                {
                    if (this.Controls.ContainsKey(txtstrc))
            ...
            if (GTRs.Count == 0)
                pan_Translations.Height = 0;
            if (!String.IsNullOrEmpty(SwitcherUI.AutoCopyTranslation))
            {
                if (SwitcherUI.AutoCopyTranslation.ToLower() == gtr.targ_lang.ToLower())
                {
                    Debug.WriteLine("AutoCopyTranslation: " + gtr.targ_lang);
                    KMHook.RestoreClipBoard(gtr.translation);
                    SwitcherUI.ACT_Match++;
                }
            }
            ...
            else
            {
                ...
                pan.Height = SwitcherUI.TrText.Height * 2;
                ...
                var slt = new SwitcherUI.TextBoxCA();
                var txt = new SwitcherUI.TextBoxCA();
                ...
                txt.Text = SwitcherUI.UnescapeUnicode(gtr.translation);
                ...
                if (TRANSCRIPTION)
                {
                    ...
                    {
                        SwitcherUI.TextBoxCA txttrc = new SwitcherUI.TextBoxCA();
                        txttrc.ReadOnly = true;
                        txttrc.TabStop = false;
                        txttrc.BorderStyle = 0;
                        txttrc.Multiline = true;
                        txttrc.Font = SwitcherUI.TrText;
                        txttrc.Name = "TRC_TXT" + gtr.targ_lang;
                        txttrc.Text = "[" + SwitcherUI.UnescapeUnicode(gtr.targ_transcr) + "]";
                ...
                txt_Source.Font = slt.Font = txt.Font = SwitcherUI.TrText;
                UpdateHeight();
            }
            SetOptimalWidth();
            SetOptimalWidth();
        ...
        public void UpdateTranslation(GTResp gtr)
        {
            foreach (Control ct in pan_Translations.Controls)
            {
                ...
                if (!ab)
                {
                    var txttrc = (SwitcherUI.TextBoxCA)pan.Controls[2];
                    abw = txttrc.Width;
                }
        ...
        public void UpdateApperence(Color back, Color fore, int opacity, Font font)
        {
            foreach (Control ct in pan_Translations.Controls)
            {
                if (ct.Name.Contains("PN_LINE"))
                {
                    ...
                    if (!ab)
                    {
                        var txttrc = (SwitcherUI.TextBoxCA)ct.Controls[2];
                        txttrc.ForeColor = fore;
                        txttrc.BackColor = back;
                        txt.Font = SwitcherUI.TrText;
                    }
                    ...
                    //					txt.Font = font;
                    ct.Controls[0].Font = txt.Font = SwitcherUI.TrText;
                }
            }
            txt_Source.BackColor = back;
            txt_Source.ForeColor = fore;
            txt_Source.Font = SwitcherUI.TrText;
            X.BackColor = back;
        ...
        {
            AeroCheck();
            UpdateApperence(SwitcherUI.TrBack, SwitcherUI.TrFore, SwitcherUI.TrTransparency, Font);
            if (Visible) return;
        ...
        {
            //			if (Program.switcher == null) { base.OnPaint(e); return; }
            Graphics g = CreateGraphics();
            var pn = new Pen(Color.Black);
            if (AeroEnabled && SwitcherUI.TrBorderAero)
                pn = new Pen(CurrentAeroColor());
            else
                pn.Color = SwitcherUI.TrBorder;
