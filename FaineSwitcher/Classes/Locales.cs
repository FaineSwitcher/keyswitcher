...
namespace FaineSwitcher
{
    static class Locales
    {
        ...
        public static void IfLessThan2()
        {
            if (AllList().Length < 2)
            {
                Program.switcher.icon.trIcon.BalloonTipClicked += (_, __) => Process.Start(@"C:\Windows\system32\rundll32.exe", "shell32.dll,Control_RunDLL input.dll");
                Program.switcher.icon.trIcon.ShowBalloonTip(4500, "You have too less layouts(locales/languages)!!", "This program switches texts by system's layouts(locales/languages), please add at least 2!", ToolTipIcon.Warning);
            }
        }
        /// <summary>
