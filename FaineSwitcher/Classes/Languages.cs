...
public class Languages
{
    ...
    public static Dictionary<Element, string> English = new Dictionary<Element, string>() { 
        ...
        { Element.JustIcon, "FaineSwitcher's icon" },
        { Element.ContryFlags, "Country flags" },
        { Element.TextLayout, "Text of layout" },
        { Element.BlockCtrlHKs, "Block FaineSwitcher hotkeys with Ctrl." },
        ...
        { Element.TT_OneLayout, "Allows to store global layout in FaineSwitcher, instead of layout per window/program.\r\n(if You have Windows 8 or greater this feature is built in Windows, so you don't need to use enable it in FaineSwitcher)"},
        ...
        { Element.TT_ConfigsInAppData, "If enabled FaineSwitcher will copy current configs to AppData, and will use them.\r\nAlso logs and snippets will be stored in %AppData%\\FaineSwitcher.\r\nAfter this checkbox state changed other checkboxes/datas configurations from user interface will not be saved, because they will be loaded from another configs(if switched, from FaineSwitcher's directory or from %AppData%\\FaineSwitcher).\r\nUseful if you need to run FaineSwitcher from Program Files directory by multiple users, and while some of them have no write access to it,\r\nalso it makes possible to have different configurations for each user." },
        { Element.TT_KeysType, "Select which keys type to display in FaineSwitcher user interface, they are both working at same time,\r\nso try not to set same keys/hotkeys to avoid double layout switching."},
        ...
        { Element.TT_RemapCapslockAsF18, "Remaps Caps Lock as F18, after this CapsLock will be disabled.\r\nTo toggle its state use Ctrl/Alt/Shift/Win + Caps Lock key.\r\n! FaineSwitcher window excluded from remap!\r\nIn FaineSwitcher you should set hotkeys as Caps Lock key, in other programs they will be remapped as F18.\r\nAfter changing hotkey don't forget to press Apply or OK." },
        { Element.TT_UseJKL, "Allows FaineSwitcher to retrieve layout from jkl.exe.\r\nAllows always to get right keyboard layout.(Highly recommended)\r\n" },
        { Element.TT_ReadOnlyNA, "Switches to read-only mode when no write access to FaineSwitcher.ini.\r\nUseful for Administrators." },
        { Element.TT_WriteInputHistory, "Writes input to history.txt(in FaineSwitcher folder or in AppData, depends on configuration),\nwith support of <Backspace> key!, though it may be slow if history too big." },
        { Element.TT_UseDelayAfterBackspaces, "If enabled FaineSwitcher will wait some time after deleting old word and before inputting converted word.\r\nUseful if in some apps FaineSwitcher's function to convert last word doesn't work properly." },
...
You need to specify in text box to the right code of layout in which FaineSwitcher will be switching, choose layout that doesn't have AltGr combinations: for example English(US) 67699721.
You can even specify layout which is not loaded in system, for example: English(US) 67699721, FaineSwitcher will load/unload it when pressing/releasing LCtrl+LAlt.
Specify layout code 0 to disable this feature." },
    ...
    public static Dictionary<Element, string> Ukrainian = new Dictionary<Element, string>() {
        ...
        { Element.JustIcon, "Значок FaineSwitcher" },
        ...
        { Element.ToggleSwitcher, "Перемкнути паузу FaineSwitcher" },
        ...
        { Element.UpdateSwitcher, "<- Оновити FaineSwitcher" },
        ...
        { Element.About, "Горячі клавіші: Ви можете подивитися їх в вкладці гарячі клавіші.\r\n"+
            ...
            "\r\nПочитайте wiki або запитайте мене, якщо у вас є питання щодо FaineSwitcher (ел. пошта і посилання на wiki нижче).\r\nУдачі."},
		#endregion
		#region Sync
        ...
        { Element.UpdateComplete, "FaineSwitcher успішно оновлено!" },
        { Element.ShowHide, "Показати" },
        { Element.Switcher, "FaineSwitcher – вбудований перемикач розкладок." },
        { Element.Download, "Завантажити" },
        { Element.ConfigsCannot, "Файл налаштувань FaineSwitcher.ini не може бути " },
        ...
        { Element.TT_Logging, "Призначено ТІЛЬКИ для пошуку помилок, впливає БІЛЬШЕ НІЖ НА ШВИДКІСТЬ РОБОТИ, журнали зберігаються у папці [Logs] поруч з FaineSwitcher.exe." },
        ...
        { Element.TT_OneLayout, "Дозволяє зберігати розкладку в FaineSwitcher замість розкладки для кожного вікна/програми.\r\n(якщо у вас Windows 8 і вище, то там дана функція вже стоїть за замовчуванням, немає необхідності включати її ще і в FaineSwitcher)"},
        ...
        { Element.TT_ConfigsInAppData, "Якщо включено, FaineSwitcher скопіює поточну конфігурацію (FaineSwitcher.ini) в AppData, і буде використовувати звідти.\r\nТакож логи і сніппети будуть зберігатися в %AppData%\\FaineSwitcher.\r\nКоли стан цієї галочки змінено, всі інші галочки/дані конфігурації, змінені в користувальницькому інтерфейсі, не будуть збережені,\r\nт.к. відбудеться зміна конфігурації і завантаження обраної (із папки FaineSwitcher або з %AppData%\\FaineSwitcher).\r\nКорисно, якщо потрібно запускати FaineSwitcher в папці Program Files, а прав на запис у всіх користувачів - немає,\r\nі ще це дає можливість мати різні налаштування для кожного користувача." },
        { Element.TT_KeysType, "Виберіть, який тип клавіш відображати в FaineSwitcher, ОБИДВА типи працюють ОДНОЧАСНО,\r\nтак що краще не призначати однакові клавіші/гор. клавіші для уникнення подвійного перемикання розкладки."},
        ...
        { Element.TT_RemapCapslockAsF18, "Перезаписує клавішу Caps Lock як F18, після чого функція клавіши Caps Lock буде вимкнена.\r\nЩоб перемкнути стан Caps Lock, натисніть її разом з однією з клавіш-модифікаторів: Ctrl/Alt/Shift/Win.\r\n! Вікно FaineSwitcher виключено з перезапису!\r\nУ FaineSwitcher призначайте гор. клавіші на Caps Lock, в інших програмах вона буде перезаписана як F18.\r\nПісля зміни гор. клавіш не забудьте натиснути на «Застосувати» чи «ОК»." },
        { Element.TT_UseJKL, "Отримує розкладку для FaineSwitcher за допомогою jkl.exe.\r\nДозволяє завжди відображати правильну розкладку (Строго рекомендується включити).\r\n" },
        { Element.TT_ReadOnlyNA, "Перемикається в режим «тільки читання», коли немає прав на запис FaineSwitcher.ini.\r\nКорисно для Адміністраторів." },
        { Element.TT_WriteInputHistory, "Буде писати історію введення в history.txt (в папці FaineSwitcher або в AppData – в залежності від налаштувань),\nз підтримкою клавіші <Backspace>!, хоча може працювати повільно, якщо історія занадто велика." },
        { Element.TT_UseDelayAfterBackspaces, "Якщо включено, FaineSwitcher буде чекати деякий час після видалення старого слова і перед введенням конвертованого слова.\r\nКорисно, якщо в деяких програмах функція FaineSwitcher «Конвертація останнього слова» не працює нормально." },
...
Можна вказати навіть ту розкладку, яка не завантажена в системі, наприклад English (US) = 67699721; FaineSwitcher завантажить/вивантажить її при натисканні/відпусканні LCtrl+LAlt.
Вкажіть код розкладки 0, щоб вимкнути цю функцію." },
    ...
    public static Dictionary<Element, string> Russian = new Dictionary<Element, string>() {
        ...
        { Element.ToggleSwitcher, "Переключать паузу FaineSwitcher" },
        ...
        { Element.UpdateSwitcher, "<- Обновить FaineSwitcher" },
        ...
        { Element.About, "Горячие клавиши: Вы можете посмотреть их во вкладке горячие клавиши.\r\n"+
            ...
            "\r\nПочитайте wiki или спросите меня, если у вас есть возникли вопросы по поводу FaineSwitcher (эл. почта и ссылка на wiki ниже).\r\nУдачи."}, 
		#endregion
		#region Sync
        ...
        { Element.UpdateComplete, "FaineSwitcher успешно обновлен!" },
        { Element.ShowHide, "Показать" },
        { Element.Switcher, "FaineSwitcher (魔法) – волшебный переключатель раскладок." },
        { Element.Download, "Скачать" },
        { Element.ConfigsCannot, "Файл настроек FaineSwitcher.ini не может быть " },
        ...
        { Element.TT_Logging, "Предназначено ТОЛЬКО для поиска ошибок, оказывает БОЛЬШОЕ ВЛИЯНИЕ НА СКОРОСТЬ РАБОТЫ, журналы сохраняются в папке [Logs] рядом с FaineSwitcher.exe." },
        ...
        { Element.TT_OneLayout, "Позволяет хранить раскладку в FaineSwitcher вместо раскладки для каждого окна/программы.\r\n(если у вас Windows 8 и выше, то там данная функция уже стоит по умолчанию, нет необходимости включать ее ещё и в FaineSwitcher)"},
        ...
        { Element.TT_ConfigsInAppData, "Если включено, FaineSwitcher скопирует текущую конфигурацию (FaineSwitcher.ini) в AppData, и будет использовать оттуда.\r\nТакже логи и сниппеты будут храниться в %AppData%\\FaineSwitcher.\r\nКогда состояние этой галочки изменено, все другие галочки/данные конфигурации, измененные в пользовательском интерфейсе, не будут сохранены,\r\nт.к. произойдет смена конфигурации и загрузка выбранной (из папки FaineSwitcher или из %AppData%\\FaineSwitcher).\r\nПолезно, если нужно запускать FaineSwitcher в папке Program Files, а прав на запись у всех пользователей - нет,\r\nи еще это даёт возможность иметь разные настройки для каждого пользователя." },
        { Element.TT_KeysType, "Выберите, какой тип клавиш отображать в FaineSwitcher, ОБА типа работают ОДНОВРЕМЕННО,\r\nтак что лучше не назначайте одинаковые клавиши/гор. клавиши во избежание двойного переключения раскладки."},
        ...
        { Element.TT_RemapCapslockAsF18, "Переопределяет клавишу Caps Lock как F18, после чего функция клавиши Caps Lock будет отключена.\r\nЧтобы переключить состояние Caps Lock, нажмите её вместе с одной из клавиш-модификаторов: Ctrl/Alt/Shift/Win.\r\n! Окно FaineSwitcher исключено из переопределения!\r\nВ FaineSwitcher назначайте гор. клавиши на Caps Lock, в других программах она будет переопределена как F18.\r\nПосле смены гор. клавиш не забудьте нажать на «Применить» или «ОК»." },
        { Element.TT_UseJKL, "Получает раскладку для FaineSwitcher с помощью jkl.exe.\r\nПозволяет всегда отображать правильную раскладку (Настоятельно рекомендуется включить).\r\n" },
        { Element.TT_ReadOnlyNA, "Переключается в режим «только чтение», когда нет прав на запись FaineSwitcher.ini.\r\nПолезно для Администраторов." },
        { Element.TT_WriteInputHistory, "Будет писать историю ввода в history.txt (в папке FaineSwitcher или в AppData – в зависимости от настроек),\nс поддержкой клавиши <Backspace>!, хотя может работать медленно, если история слишком большая." },
        { Element.TT_UseDelayAfterBackspaces, "Если включено, FaineSwitcher будет ждать некоторое время после удаления старого слова и перед вводом конвертированного слова.\r\nПолезно, если в некоторых программах функция FaineSwitcher «Конвертация последнего слова» не работает нормально." },
...
Можно указать даже ту раскладку, которая не загружена в системе, например English (US) = 67699721; FaineSwitcher загрузит/выгрузит её при нажатии/отпускании LCtrl+LAlt.
