...
namespace FaineSwitcher
{
    static class KInputs
    {
        ...
        {
            var WORDS = FaineSwitcher.KMHook.SplitWords(LINE);
            string word = "";
            var len = 0;
        ...
        public static WinAPI.INPUT[] AddString(string str)
        {
            ...
            foreach (var s in str)
            {
                ...
                ushort resultvk = 0;
                short lt1_vk = WinAPI.VkKeyScanEx(s, FaineSwitcher.SwitcherUI.MAIN_LAYOUT1);
                uselt1_vk = lt1_vk != -1;
                short lt2_vk = WinAPI.VkKeyScanEx(s, FaineSwitcher.SwitcherUI.MAIN_LAYOUT2);
                uselt2_vk = lt2_vk != -1;
                if (uselt1_vk && uselt2_vk && lt1_vk != lt2_vk)
                {
                    var guess = FaineSwitcher.KMHook.WordGuessLayout(GetWordByIndex(str, index));
                ...
                var down = new WinAPI.INPUT
                {
                    ...
                    Data = {
                        Keyboard = new WinAPI.KEYBDINPUT {
                            Vk = SwitcherUI.GuessKeyCodeFix ? resultvk : (ushort)0,
            ...
            if (SwitcherUI.GuessKeyCodeFix)
            {
                if (l1_ok != l2_ok)
                {
                    var lll = (l1_ok > l2_ok ? SwitcherUI.MAIN_LAYOUT1 : SwitcherUI.MAIN_LAYOUT2);
        ...
        { //Simply, sends input
           var rinputs = new List<WinAPI.INPUT>();
            ...
            if (done != sinputs.Length)
                FaineSwitcher.Logging.Log("ERROR during send input, lenght: " + done + ", Win32ERR: " + Marshal.GetLastWin32Error());
        }
    }
}