...
namespace FaineSwitcher
{
    static class jklXHidServ
    {
        ...
        const string HWND_SERVER_TITLE = "777_Switcher_777_HIDDEN_HWND_SERVER";
        ...
        public static bool jklExist()
        {
            ...
            if (jklFEX[0] && jklFEX[1] && jklFEX[2] && jklFEX[3])
                return true;
            jklInfoStr = "jkl.exe " + (jklFEX[0] ? "" : Program.Lang[Languages.Element.Not] + " ") + Program.Lang[Languages.Element.Exist] + "\r\n";
            jklInfoStr += "jkl.dll " + (jklFEX[1] ? "" : Program.Lang[Languages.Element.Not] + " ") + Program.Lang[Languages.Element.Exist] + "\r\n";
            jklInfoStr += "jklx86.exe " + (jklFEX[2] ? "" : Program.Lang[Languages.Element.Not] + " ") + Program.Lang[Languages.Element.Exist] + "\r\n";
            jklInfoStr += "jklx86.dll " + (jklFEX[3] ? "" : Program.Lang[Languages.Element.Not] + " ") + Program.Lang[Languages.Element.Exist] + "\r\n";
            return false;
        }
                ...
                if (jklExist())
                {
                    ...
                    for (; ii != umsgIDs.Length; ii++)
                    {
                        ...
                        if (tries <= 20)
                        {
                            ...
                            KMHook.DoLater(() => { SwitcherUI.GlobalLayout = SwitcherUI.currentLayout = Locales.GetCurrentLocale(); }, 200);
                            running = true;
                        }
                    }
                }
        ...
        {
            self_change = true;
            //			for (int i = 0; i!=Program.PHLayouts; i++) {
            //				if (Program.SwitcherActive()) return; // Else creates invalid culture 0 exception.
            WinAPI.SendMessage(hwnd, (int)WinAPI.WM_INPUTLANGCHANGEREQUEST, 0, WinAPI.HKL_NEXT);
            //				Logging.Log("[JKL] > Cycle all: "+i+"/"+Program.PHLayouts);
            //				Thread.Sleep(5);
            ...
            protected override void WndProc(ref Message m)
            {
                if (m.Msg == jkluMSG[0] || m.Msg == jkluMSG[1])
                {
                    ...
                    else
                    {
                        ...
                        SwitcherUI.GlobalLayout = SwitcherUI.currentLayout = layout;
                        ...
                        if (start_cyclEmuSwitch)
                        {
                            ...
                            else
                            {
                                jklChangeTries = 0;
                                start_cyclEmuSwitch = false;
                                if (KMHook.CompareLayouts(SwitcherUI.MAIN_LAYOUT1, layout) ||
                                    KMHook.CompareLayouts(SwitcherUI.MAIN_LAYOUT2, layout))
                                {
                                    KMHook.last_switch_layout = layout;
                                    KMHook.evt_layoutchanged(layout, 0, SwitcherUI.bindable_events[0]);
                                }
                            }
                        }
