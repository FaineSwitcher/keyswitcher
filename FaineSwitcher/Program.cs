    ...
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AppDomain.CurrentDomain.UnhandledException += (obj, arg) =>
            {
                ...
                Logging.Log("Unexpected error occurred, FaineSwitcher exited, error details:\r\n" + e.Message + "\r\n" + e.StackTrace, 1);
            };
            ...
            Logging.Log("FaineSwitcher started.");
            var ind = 0;
            using (var mutex = new Mutex(false, GGPU_Mutex))
            {
                ...
                {
                    var Switcher_folder_appd = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "FaineSwitcher");
                    if (!Directory.Exists(Switcher_folder_appd))
                        Directory.CreateDirectory(Switcher_folder_appd);
                    if (!File.Exists(Path.Combine(Switcher_folder_appd, "FaineSwitcher.ini"))) // Copy main configs to appdata
                        File.Copy(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "FaineSwitcher.ini"),
                                  Path.Combine(Switcher_folder_appd, "FaineSwitcher.ini"), true);
                    Configs.filePath = Path.Combine(Switcher_folder_appd, "FaineSwitcher.ini");
                    MyConfs = new Configs();
                }
                else
                    Configs.filePath = Path.Combine(SwitcherUI.nPath, "FaineSwitcher.ini");
                ...
                if (args.Length != 0)
                {
                    ...
                    {
                        Logging.Log("FaineSwitcher updated.");
                    ...
                    {
                        Logging.Log("FaineSwitcher silently updated.");
                        switcher.icon.trIcon.Visible = true;
                        switcher.icon.trIcon.ShowBalloonTip(1000, Lang[Languages.Element.UpdateComplete], "FaineSwitcher -> " + switcher.Text, ToolTipIcon.Info);
        ...
        public static bool SwitcherActive()
        {
            ...
            Logging.Log("FaineSwitcher is active = [" + active + "]" + ", FaineSwitcher handle [" + switcher.Handle + "], fore win handle [" + ActHandle + "]");
            return active;
        }
    }
}